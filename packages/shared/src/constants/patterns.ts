...
const EXTRA_VITA_COMPONENT_PATTERNS: PatternDefinition[] = VITA_COMPONENT_PATTERN_NUMBERS.map((patternNumber) => ({
  ...
  rowMultiple: 1,
  columnMultiple: 1,
  displayName: `Vita Pattern ${patternNumber}`,
  ...
  defaultUnitHeight: patternNumber === 14 ? 636.416 : 610,
...
const ALL_PATTERN_CATALOG: PatternDefinition[] = [
  ...
  },
  {
    ...
    defaultUnitWidth: 50.21,
    defaultUnitHeight: 149.16,
    svgRepeatWidth: 50.21,
    svgRepeatHeight: 149.16,
    repeatCompensation: {
      x: -25,
      y: -80,
    },
  ...
  },
  {
    ...
    defaults: { rows: 1, columns: 1, angle: 0, stretchers: 1, weaves: 1, embossDepth: 100 },
    parameterRanges: {
      rows: { min: 1, max: 10 },
      columns: { min: 1, max: 10 },
      angle: { min: 0, max: 0, step: 1 },
