...
const PATTERN_MAP_FILE = path.join(PATTERN_DIR, '_pattern-map.json');
const OUTPUT_DIR = path.join(ROOT, 'app/(app)/create/engine/generated/svg-pattern-modules');
const OUTPUT_MODULE_DIR = path.join(OUTPUT_DIR, 'modules');
const LEGACY_OUTPUT_FILE = path.join(ROOT, 'app/(app)/create/engine/generated/svg-pattern-modules.ts');
...
async function generate() {
  const modules = {};
  const diagnostics = [];
  const patternMap = JSON.parse(await fs.readFile(PATTERN_MAP_FILE, 'utf8'));

  for (const [patternType, filename] of Object.entries(patternMap)) {
  ...
  await fs.mkdir(OUTPUT_MODULE_DIR, { recursive: true });
  await fs.rm(LEGACY_OUTPUT_FILE, { force: true });
...
`;
  await fs.writeFile(path.join(OUTPUT_DIR, 'types.ts'), typesContent, 'utf8');

  const loaderEntries = Object.keys(modules)
    .map(
      (patternType) =>
        `  ${JSON.stringify(patternType)}: () => import('./modules/${patternType}').then((mod) => mod.default),`,
    )
    .join('\n');
  const loadersContent = `${header}
...
export const SVG_PATTERN_MODULE_TYPES = ${JSON.stringify(Object.keys(modules), null, 2)} as const;
...
`;
  ...
  for (const [patternType, moduleData] of Object.entries(modules)) {
    const moduleContent = `${header}

import type { SvgPatternModule } from '../types';

const svgPatternModule: SvgPatternModule = ${JSON.stringify(moduleData, null, 2)};

export default svgPatternModule;
`;

    await fs.writeFile(path.join(OUTPUT_MODULE_DIR, `${patternType}.ts`), moduleContent, 'utf8');
  }
