...
describe('editor store', () => {
  ...
  it('applies 5 mm linked joints for Vita Pattern 3 by default', () => {
    ...
    useEditorStore.getState().setPatternType('vita_pattern_3');

    const { config } = useEditorStore.getState();

    expect(config.pattern.type).toBe('vita_pattern_3');
  ...
  it('applies 40 mm linked joints for Vita Pattern 10 by default', () => {
    ...
    useEditorStore.getState().setPatternType('vita_pattern_10');

    const { config } = useEditorStore.getState();

    expect(config.pattern.type).toBe('vita_pattern_10');
    expect(config.joints.horizontalSize).toBe(40);
    expect(config.joints.verticalSize).toBe(40);
    expect(config.joints.linkedDimensions).toBe(true);
  });

  it('applies 40 mm linked joints for Vita Pattern 18 by default', () => {
    useEditorStore
      .getState()
      .loadProjectConfig(structuredClone(DEFAULT_TEXTURE_CONFIG), { resetHistory: true });

    useEditorStore.getState().setPatternType('vita_pattern_18');

    const { config } = useEditorStore.getState();

    expect(config.pattern.type).toBe('vita_pattern_18');
    expect(config.joints.horizontalSize).toBe(40);
    expect(config.joints.verticalSize).toBe(40);
    expect(config.joints.linkedDimensions).toBe(true);
  });
